...
def movie_selection(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error fetching download links for {movie_url} from {site}: {str(e)}")
            query.message.edit_text(f"Error fetching download links: {str(e)}. Try again later.")
            return MOVIE_SELECTION
